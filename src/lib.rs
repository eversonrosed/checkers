...
mod tests {
  ...
  use crate::game::Color::{White, Black};
  ...
  fn pieces_move() {
    ...
    assert_eq!(rmbb, piece_moves(White, false, Bitboard::from(1 << 18)));
    let bmbb = Bitboard::from((1 << 9) | (1 << 11));
    assert_eq!(bmbb, piece_moves(Black, false, Bitboard::from(1 << 18)));
    let kbb = Bitboard::from((1 << 9) | (1 << 11) | (1 << 25) | (1 << 27));
    assert_eq!(kbb, piece_moves(White, true, Bitboard::from(1 << 18)));
    assert_eq!(kbb, piece_moves(Black, true, Bitboard::from(1 << 18)));
  }
